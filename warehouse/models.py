...
class Stock(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            # Одна строка остатка на позицию склада — но ограничение
            # приходится задавать двумя половинами, и вот почему.
            #
            # Прежде тут стояло одно ограничение на тройку полей
            # (склад, материал, продукция). Оно не работало никогда: у
            # строки остатка одно из двух полей всегда пустое —
            # у материала пустая продукция, у продукции пустой материал,
            # — а в SQL пустое значение не равно другому пустому. База
            # считала такие строки разными и пропускала дубли.
            #
            # Замечено это было не на глаз: два одновременных прихода на
            # позицию, которой ещё нет на складе, стабильно создавали две
            # строки остатка вместо одной. Дальше позиция ломалась
            # насовсем — списание ищет одну строку, находит две и падает
            # с ошибкой сервера.
            #
            # Половинки берут в расчёт только заполненное поле, поэтому
            # пустые значения сравнивать не приходится. Работает и на
            # SQLite, и на PostgreSQL. Заодно это чинит саму гонку:
            # get_or_create, наткнувшись на запрет базы, перечитывает
            # чужую строку вместо того, чтобы завести свою.
            models.UniqueConstraint(
...
class Order(models.Model):
    ...
    status = models.CharField(
        ...
        db_index=True)
    created_at = models.DateTimeField('Создан', auto_now_add=True)
    # Откуда пришёл заказ. Нужен, чтобы ограничить частоту: без него
    # один человек за минуту оформляет сотню заказов, весь товар
    # уходит в резерв и на витрине показывается «нет в наличии».
    # Хранится только у заказов с витрины; заказ, заведённый
    # сотрудником, остаётся без адреса.
    created_ip = models.GenericIPAddressField(
        'Адрес, откуда оформлен', null=True, blank=True)
