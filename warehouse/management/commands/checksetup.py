    ...
    def _check_https(self):
        ...
        if not getattr(settings, 'USE_HTTPS', False):
            if not settings.DEBUG:
                ...
                # Тот, кто уже включал HTTPS и передумал, натыкается на
                # это почти наверняка. Куки, выданные при включённом
                # HTTPS, помечены «только по защищённому соединению», и
                # браузер держит их до последнего: по http он их не
                # шлёт, а заменить их страница по http не вправе —
                # таково правило самих браузеров. Выходит тупик, который
                # переживает и правку настроек, и перезапуск, и очистку
                # кэша: чистить надо именно куки.
                self.note(
                    'Если при входе появляется «Ошибка проверки CSRF», а '
                    'HTTPS вы недавно выключали — дело в куках, оставшихся '
                    'с той поры: они помечены «только по защищённому '
                    'соединению» и по http не работают. Удалите в браузере '
                    'куки для адреса системы (в Chrome и Edge — Ctrl+Shift+'
                    'Delete, пункт «Файлы cookie и другие данные сайтов»). '
                    'Проверить догадку быстрее всего в окне InPrivate: там '
                    'старых кук нет.')
            return

        hsts = getattr(settings, 'SECURE_HSTS_SECONDS', 0)
        self.ok(f'Включён режим HTTPS: перенаправление, защищённые куки, '
                f'HSTS на {hsts // 86400} дн.')

        # Самая частая беда с HTTPS — не в настройках, а в том, что его
        # включили раньше, чем он заработал. Снаружи это выглядит как
        # «сайт не может обеспечить безопасное подключение», и по такому
        # сообщению причину не угадать. Поэтому подсказка выводится
        # всегда, когда режим включён, а не только при поломке.
        self.note(
            'Если система перестала открываться и браузер пишет '
            '«не может обеспечить безопасное подключение» '
            '(ERR_SSL_PROTOCOL_ERROR) — HTTPS включён, а на деле не '
            'работает. Впишите в .env USE_HTTPS=False, перезапустите и '
            'откройте заново.')
        self.note(
            'Второй признак той же беды: страница входа открывается, но '
            'при нажатии «Войти» отвечает «Ошибка проверки CSRF». При '
            'включённом HTTPS куки помечаются «только по защищённому '
            'соединению», и по обычному http браузер их не сохраняет — '
            'входить нечем. Лечится так же: USE_HTTPS=False, либо '
            'открывайте систему по https, а не по http.')

        if hsts:
            self.note(
                f'Браузер будет помнить про https {hsts // 86400} дн. '
                f'Пока сертификат обкатывается, поставьте в .env '
                f'HSTS_SECONDS=0 — тогда вернуться к http можно в любой '
                f'момент, не трогая настройки браузера.')
