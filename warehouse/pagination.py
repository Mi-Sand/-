...
и запись в settings ни на что не влияет. Без класса запрос ?limit=1000000
...
class WarehouseLimitOffsetPagination(LimitOffsetPagination):
    ...
    # Клиентская функция apiCallAll забирает страницы подряд по ссылке next,
    # поэтому потолок не мешает выгрузить всё — только не разом.
    max_limit = 500
