...
# Своя страница отказа при неудачной проверке подлинности запроса.
#
# Django отвечает на такой отказ одной строкой — «Ошибка проверки CSRF.
# Запрос отклонён», — и по ней нельзя понять ни причины, ни что делать.
# А причина почти всегда одна из двух, и обе видны прямо в запросе:
# либо перед системой стоит nginx с шифрованием, о котором ей не
# сказали, либо браузер держит куки, оставшиеся от прежнего HTTPS.
# Разбор — в warehouse/csrf.py.
CSRF_FAILURE_VIEW = 'warehouse.csrf.csrf_failure'
