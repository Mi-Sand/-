...
from accounts.permissions import CanProcessOrders
from warehouse.views import TextSearchMixin
from warehouse.models import Order
from warehouse.params import read_date
...
def _parse_date(value, default):
    ...
    В отличие от отборов на других страницах, негодная дата здесь не
    пропускается молча: акт сверки — документ на подпись, и период в
    нём должен быть тем, который задал сотрудник, а не подставленным вместо
    него.
    """
    if value in (None, ''):
        return default
    parsed = read_date(str(value))
    if parsed is None:
        raise ValueError(f'Неверная дата: {value!r}. Ожидается ГГГГ-ММ-ДД')
    return parsed


class CounterpartyViewSet(TextSearchMixin, viewsets.ModelViewSet):
    """Справочник покупателей-организаций.

    Права — те же, что на работу с заказами: карточка покупателя
    заводится ради счёта или накладной, и заводит её тот, кто заказ
    ведёт. Читать может любой вошедший сотрудник.
    """

    queryset = Counterparty.objects.all()
    serializer_class = CounterpartySerializer
    permission_classes = [CanProcessOrders]
    text_search_fields = ('short_name', 'inn', 'legal_address')
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    Оплата — денежная запись, и вводит её тот же, кто ведёт заказы.
    Правка чужой оплаты задним числом видна в журнале действий.
    """
