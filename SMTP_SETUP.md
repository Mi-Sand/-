...
1. **Ежедневная сводка** — раз в сутки одним письмом: остатки ниже минимума,
   истекающие сроки, заказы без подтверждения, свежесть резервной копии, а
   также что происходило за сутки (`manage.py dailysummary`).
2. **Недостачи при инвентаризации** — сразу по итогам, если выявлены
   существенные недостачи (`/inventory/services.py`).
3. **Низкий остаток материала** — по умолчанию **выключено**. Остаток
   пересчитывается при каждом проведении документа, и при десятке материалов
   набегал десяток писем в день; такой ящик перестают читать. Низкие остатки
   теперь перечислены в сводке. Включается в `.env`:
   `NOTIFY_LOW_STOCK_INSTANTLY=True` (`/warehouse/signals.py`).
...
Команда показывает нынешние настройки (пароль не показывает) и отправляет
одно письмо. Если почтовый сервер откажет, она объясняет причину по-русски:
не тот порт, не то шифрование, не подошёл пароль. Проверьте и папку «Спам» —
письма от новых отправителей часто попадают туда.
